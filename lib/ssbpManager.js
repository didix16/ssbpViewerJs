(function(){
	...
	window.lib.ssbpManager = {
		...
		getAnimePacks: function(){
			...
			for(var i = 0; i < len; ++i){
				...
				animePack.animations = this.getAnimations(animePack.animationsAdress, animePack.numAnimations);
		...
		getAnimations: function(offset, num){
			...
			for(var i = 0; i < num; ++i){
				...
				anim.defaultData = this.getAnimationInitialData(anim.defaultDataAddress);
		...
		getAnimationInitialData: function(offset){
	...
	lib.ssbpManager.DataArrayReader = function(data, addr){
		...
		this.readS16 =  function(){
			var d = this.data.getInt16(this.addr, this.ENDIAN) & 0xFFFF;
			this.addr += 2;
			return d;
		};
		...
		this.readS32 = function(){
			...
			var d = this.data.getInt32(this.addr, this.ENDIAN) & 0xFFFFFFFF;
			this.addr += 4;
			return d;
		};
		...
		{
			var d = this.data.getFloat32(offset, this.ENDIAN);
			this.addr += 4;
			return d;
		};
	...
	lib.ssbpManager.Player.SpriteSheet = function (name, src) {
		...
		this.image.onload = function (event) {
			...
			console.info("Image loaded. Getting cells...");
			var $ssbp = lib.ssbpManager;
			...
			for(var i = 0; i < cells.length; ++i){
				...
				var cell = cells[i];
				var imgdata = ctx.getImageData(cell.x, cell.y, cell.width, cell.height);
				buffer.width = cell.width;
				buffer.height = cell.height;

				buffer.putImageData(imgdata);
				var c = new Image();
				c.src = buffer.toDataURL();
				this.cells.push(c);
			}
