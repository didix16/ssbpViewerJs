(function(){
	...
	lib.ssbpManager.State = function(){

		this.flags = 0;						/// このフレームで更新が行われるステータスのフラグ
		this.cellIndex = 0;					/// パーツに割り当てられたセルの番号
		this.x = 0.0;						/// SS5アトリビュート：X座標
		this.y = 0.0;						/// SS5アトリビュート：Y座標
		this.z = 0.0;						/// SS5アトリビュート：Z座標
		this.pivotX = 0.0;					/// 原点Xオフセット＋セルに設定された原点オフセットX
		this.pivotY = 0.0;					/// 原点Yオフセット＋セルに設定された原点オフセットY
		this.rotationX = 0.0;				/// X回転（親子関係計算済）
		this.rotationY = 0.0;				/// Y回転（親子関係計算済）
		this.rotationZ = 0.0;				/// Z回転（親子関係計算済）
		this.scaleX = 1.0;					/// Xスケール（親子関係計算済）
		this.scaleY = 1.0;					/// Yスケール（親子関係計算済）
		this.opacity = 255;					/// 不透明度（0～255）（親子関係計算済）
		this.size_X = 1.0;					/// SS5アトリビュート：Xサイズ
		this.size_Y = 1.0;					/// SS5アトリビュート：Xサイズ
		this.uv_move_X = 0.0;				/// SS5アトリビュート：UV X移動
		this.uv_move_Y = 0.0;				/// SS5アトリビュート：UV Y移動
		this.uv_rotation = 0.0;				/// SS5アトリビュート：UV 回転
		this.uv_scale_X = 1.0;				/// SS5アトリビュート：UV Xスケール
		this.uv_scale_Y = 1.0;				/// SS5アトリビュート：UV Yスケール
		this.boundingRadius = 0.0;			/// SS5アトリビュート：当たり半径
		this.colorBlendFunc = 0;				/// SS5アトリビュート：カラーブレンドのブレンド方法
		this.colorBlendType = 0;				/// SS5アトリビュート：カラーブレンドの単色か頂点カラーか。
		this.flipX = false;						/// 横反転（親子関係計算済）
		this.flipY = false;						/// 縦反転（親子関係計算済）
		this.isVisibled = false;				/// 非表示（親子関係計算済）
		this.instancerotationX = 0.0;		/// インスタンスパーツに設定されたX回転
		this.instancerotationY = 0.0;		/// インスタンスパーツに設定されたY回転
		this.instancerotationZ = 0.0;		/// インスタンスパーツに設定されたZ回転
		this.quad = null;		/// 頂点データ、座標、カラー値、UVが含まれる（頂点変形、サイズXY、UV移動XY、UVスケール、UV回転、反転が反映済）
		this.texture = null;			/// セルに対応したテクスチャ番号（ゲーム側で管理している番号を設定する）
		this.rect = null;					/// セルに対応したテクスチャ内の表示領域（開始座標、幅高さ）
		this.blendfunc = 0;					/// パーツに設定されたブレンド方法
		this.mat = []; // [16]					/// パーツの位置を算出するためのマトリクス（親子関係計算済）
	...
	lib.ssbpManager.CustomSprite = function(){
		...
		this.create = function(){
			...
			return new lib.ssbpManager.CustomSprite();
		}
		...
		this.setStateValue = function(param, value){
		...
		this.setState = function(state){
			...
			this
				.setStateValue("flags", state.flags)
				.setStateValue("cellIndex", state.cellIndex)
				.setStateValue("x", state.x)
				.setStateValue("y", state.y)
				.setStateValue("z", state.z)
				.setStateValue("pivotX", state.pivotX)
				.setStateValue("pivotY", state.pivotY)
				.setStateValue("rotationX", state.rotationX)
				.setStateValue("rotationY", state.rotationY)
				.setStateValue("rotationZ", state.rotationZ)
				.setStateValue("scaleX", state.scaleX)
				.setStateValue("scaleY", state.scaleY)
				.setStateValue("opacity", state.opacity)
				.setStateValue("size_X", state.size_X)
				.setStateValue("size_Y", state.size_Y)
				.setStateValue("uv_move_X", state.uv_move_X)
				.setStateValue("uv_move_Y", state.uv_move_Y)
				.setStateValue("uv_rotation", state.uv_rotation)
				.setStateValue("uv_scale_X", state.uv_scale_X)
				.setStateValue("uv_scale_Y", state.uv_scale_Y)
				.setStateValue("boundingRadius", state.boundingRadius)
				.setStateValue("isVisibled", state.isVisibled)
				.setStateValue("flipX", state.flipX)
				.setStateValue("flipY", state.flipY)
				.setStateValue("blendfunc", state.blendfunc)
				.setStateValue("colorBlendFunc", state.colorBlendFunc)
				.setStateValue("colorBlendType", state.colorBlendType)

				.setStateValue("instancerotationX", state.instancerotationX)
				.setStateValue("instancerotationY", state.instancerotationY)
				.setStateValue("instancerotationZ", state.instancerotationZ)
				.setStateValue("quad", state.quad);

				this._state.texture = state.texture;
				this._state.rect = state.rect;
				this.mat = state.mat;
	...
	lib.ssbpManager.Player = function (elem) {
		...
		this.PART_VISIBLE_MAX = 512;

		this.canvasElem = null;
		this.spritesheets = [];
		this._parts = [];
		this._currentAnimeRef = null;
		this._playingFrame = 0;
		this._step = 1.0;
		this._loop = 0;
		this._loopCount = 0;
		this._isPlaying = false;
		this._isPausing = false;
		this._prevDrawFrameNo = -1;
		this._InstanceAlpha = 255;
		this._InstanceRotX = 0.0;
		this._InstanceRotY = 0.0;
		this._InstanceRotZ = 0.0;
		this._gamefps = 1000 / 60;
		this._col_r = 255;
		this._col_g = 255;
		this._col_b = 255;
		this._instanceOverWrite = false;
		this._motionBlendPlayer = null;
		this._blendTime = 0.0;
		this._blendTimeMax = 0.0;

		this._currentdataKey = "";
		this._currentAnimename = "";

		this._partVisible = [];
		this._state = new lib.ssbpManager.State();

		for (var i = 0; i < this.PART_VISIBLE_MAX; i++)
		{
			this._partVisible.push(true);
			//_partIndex[i] = -1;
			//_cellChange[i] = -1;
		}
		...
		this.allocParts = function(numParts){
			...
			for(var i = 0; i < numParts; ++i){
				...
				var sprite = $ssbp.CustomSprite.create();
		...
		this.setPartsParentage = function(){

			if(! this._currentAnimeRef) return;

			var $ssbp = lib.ssbpManager;

			var packData = this._currentAnimeRef.animePackData;
			var parts = packData.parts;

			for(var partIndex = 0; partIndex < packData.numParts; partIndex++){

				var partData = parts[partIndex];

				var sprite = this._parts[partIndex];

				if(partIndex > 0){

					var parent = this._parts[partData.parentIndex];
					sprite._parent = parent;
				}else{
					sprite._parent = null;
				}

				var refanimeName = partData.refname;

				if(refanimeName != ""){

					//sprite._ssplayer = new $ssbp.Player.create();
					//sprite._ssplayer.setData(this._currentdataKey);
					//sprite._ssplayer.play(refanimeName);
				...
				if(sprite.refEffect){
					...
					sprite.refEffect = null;
				}

				var refeffectName = partData.effectfilename;
...
//				sprite->refEffect->setEffectSpriteCount(&_effectSpriteCount);	//エフェクトクラスに渡す都合上publicにしておく
		...
		this.setFrame = function(frameNo){
			...
			{
				var root = this._parts[0];
				var scaleX = root.isFlippedX() ? -1.0 : 1.0;
				var scaleY = root.isFlippedY() ? -1.0 : 1.0;

				root.setStateValue("x",scaleX);
				root.setStateValue("y", scaleY);
			...
			var animeData = this._currentAnimeRef.animationData;
		...
		this.getMaxFrame = function(){

			if(this._currentAnimeRef){

				return this._currentAnimeRef.animationData.numFrames;
			}else return 0;
		};

		this.getFrameNo = function(){

			return this._playingFrame;
		};

		this.setFrameNo = function(frameNo){

			this._playingFrame = frameNo;
			return this;
		};
		...
		this.clearLoopCount = function(){

			this._loopCount = 0;
			return this;
		};

		this.set_InstanceAlpha = function(alpha){

			this._InstanceAlpha = alpha;
			return this;
		};

		this.set_InstanceRotation = function(rotX, rotY, rotZ){

			this._InstanceRotX = rotX;
			this._InstanceRotY = rotY;
			this._InstanceRotZ = rotZ;
		...
		this.setRotation = function(x, y, z){
			...
			this._state.rotationX = x;
			this._state.rotationY = y;
			this._state.rotationZ = z;
		...
		this.setScale = function (x, y) {
			...
			this._state.scaleX = x;
			this._state.scaleY = y;
		...
		this.setAlpha = function (alpha) {
			...
			this._state.opacity = alpha;

			return this;
		};

		this.setFlip = function(flipX, flipY){

			this._state.flipX = flipX;
			this._state.flipY = flipY;

			return this;
		};

		this.setGameFPS = function(fps){

			this._gamefps = fps;
			return this;
		};

		this.parcentVal = function (val1, val2, parcent) {

			var sa = val1 - val2;
			var newval = val2 + (sa * parcent);
			return newval;
		};

		this.parcentValRot = function (val1, val2, parcent) {

			var ival1 = parseInt(val1 * 10) % 3600;
			var ival2 = parseInt(val2 * 10) % 3600;
